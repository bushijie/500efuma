...
<body class="page-header-fixed">
	...
	<a href="#" class="dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-close-others="true">
	<img alt="" style="width: 29px;height:29px;" src="<?php echo ($admin_info['headimg']); ?>"/>
	<span class="username"><?php echo ($admin_info['name']); ?></span>
	<i class="fa fa-angle-down"></i>
	</a>
	<ul class="dropdown-menu">
		<li><a href="extra_profile.html"><i class="fa fa-user"></i> 个人资料</a></li>
		<li><a href="inbox.html"><i class="fa fa-envelope"></i> 我的邮箱 <span class="badge badge-danger">3</span></a></li>
		<li class="divider"></li>
		<li><a href="javascript:;" id="trigger_fullscreen"><i class="fa fa-move"></i> 全屏</a></li>
		<li><a href="<?php echo U('Admin/lock'); ?>"><i class="fa fa-lock"></i> 注销</a></li>
		<li><a href="<?php echo U('Admin/logout'); ?>"><i class="fa fa-key"></i> 退出</a></li>
	</ul>
</li>
	...
	<ul class="page-sidebar-menu">
		...
		<?php if(is_array($sidebar)): $i = 0; $__LIST__ = $sidebar;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$bar): $mod = ($i % 2 );++$i;?><!-- active | active_bar -->
			<?php if($bar["key"] == $active_bar): ?><li class="menubar active" data-key="<?php echo ($bar["name"]); ?>">
			<?php else: ?>
				<li class="menubar" data-key="<?php echo ($bar["name"]); ?>"><?php endif; ?>
				...
				<?php if($bar["child"] != null): ?><ul class="sub-menu">
						<?php if(is_array($bar["child"])): $i = 0; $__LIST__ = $bar["child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$child): $mod = ($i % 2 );++$i;?><li  data-key="<?php echo ($child["name"]); ?>">
	...
	<div class="col-md-12">
		...
		<ul class="page-breadcrumb breadcrumb">
			...
			<li>
				<a href="#">首页</a>
				<i style="display: none;" class="fa fa-angle-right last-icon"></i>
			</li>
		...
		<!-- END PAGE TITLE & BREADCRUMB-->
			...
			<div class="row">
				<div class="col-md-12">
					<iframe id="frame" name="frame" class="content" src="<?php echo U('Admin/Home/index'); ?>" scrolling="auto" frameborder="0" ></iframe>
				</div>
			</div>
		</div>
		<!-- END PAGE -->    
	...
	<div class="footer-inner">
		&copy; <?php echo date('Y',time())?> 500efuma.
		Author: Saki 
		| 粤ICP备15001603号
	</div>
...
<script>
	jQuery(document).ready(function() {    
		...
		$("#frame").load(function(){
			changeFrame();
			task = setInterval(checkFrameStatus,100);
		});
